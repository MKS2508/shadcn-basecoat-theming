...
export class ThemeInstaller {
    ...
    if (this.previewContainer) {
      ...
      this.previewContainer.innerHTML = `
        <div class="flex items-center justify-center text-muted-foreground text-sm min-h-[60px]">
          Enter a URL to preview the theme
        </div>
      `;
    }
  ...
  private async installTheme(): Promise<void> {
    ...
    try {
      // Cache the theme as installed directly
      const cachedTheme: CachedTheme = {
        name: this.currentThemeData.name,
        url,
        data: this.currentThemeData,
        installed: true,  // Set as installed immediately
        timestamp: Date.now()
      };

      console.log(`💾 Caching theme as installed: ${this.currentThemeData.name}`);
      ...
      console.log(`✅ Theme installed and marked in IndexedDB: ${this.currentThemeData.name}`);
  ...
  private renderThemeNamesList(themeNames: string[], freshFetch: boolean): void {
    ...
    // Themes grid - 2 column layout for modal
    const themesHtml = `
      <div class="grid grid-cols-2 gap-2">
        ${themeNames.map(name => {
          // Format theme name for display (keep it shorter)
          const displayName = name.split('-').slice(0, 2).join(' ').replace(/\b\w/g, l => l.toUpperCase());
          ...
          return `
            <div class="theme-name-item border rounded-md p-2 hover:bg-accent/5 hover:border-accent transition-all" data-theme-name="${name}">
              <div class="flex flex-col">
                <div class="text-sm font-medium text-foreground truncate" title="${name}">${displayName}</div>
    ...
    previewContainer.innerHTML = `
      ${headerHtml}
      ${searchHtml}
      <div class="themes-container flex-1 overflow-y-auto overflow-x-hidden">
        <div id="themes-list">
          ${themesHtml}
        </div>
      </div>
      <div class="text-xs text-muted-foreground mt-2 pt-2 border-t flex items-center justify-between">
        <span>TweakCN Registry</span>
    ...
    const previewButtons = previewContainer.querySelectorAll('.preview-theme-btn');
    previewButtons.forEach(button => {
      button.addEventListener('click', async (e) => {
        e.stopPropagation();
        const themeName = (e.target as HTMLElement).getAttribute('data-theme-name');
        if (themeName) {
          await this.previewThemeFromRegistry(themeName);
        }
      });
    });
  ...
  private filterThemeNames(themeNames: string[], query: string): void {
    ...
    // Rebuild grid with filtered themes
    const themesHtml = `
      <div class="grid grid-cols-2 gap-2">
    ...
    // Re-setup preview and install buttons
    const previewButtons = list.querySelectorAll('.preview-theme-btn');
    previewButtons.forEach(button => {
      button.addEventListener('click', async (e) => {
        e.stopPropagation();
        const themeName = (e.target as HTMLElement).getAttribute('data-theme-name');
  ...
  /**
   * Preview theme from registry without installing
   */
  private async previewThemeFromRegistry(themeName: string): Promise<void> {
    try {
      // Get the theme URL from fetcher
      const themeUrl = this.themeListFetcher.getThemeInstallUrl(themeName);
      
      // Update button state
      const button = document.querySelector(`.preview-theme-btn[data-theme-name="${themeName}"]`) as HTMLElement;
      if (button) {
        button.textContent = '⏳';
        (button as HTMLButtonElement).disabled = true;
      }
      
      // Set URL in input for preview
      if (this.urlInput) {
      ...
      previewMsg.className = 'text-xs text-accent-foreground bg-accent/20 p-2 rounded mt-2';
      previewMsg.textContent = `Previewing ${themeName} - Click "Install Theme" above to install`;
      
      if (this.previewContainer && !this.previewContainer.querySelector('.preview-msg')) {
        const existingMsg = this.previewContainer.querySelector('.text-xs.text-accent-foreground');
        if (existingMsg) existingMsg.remove();
        previewMsg.classList.add('preview-msg');
        this.previewContainer.appendChild(previewMsg);
      }
      
    } catch (error) {
      console.error(`❌ Failed to preview theme ${themeName}:`, error);
      ...
      this.showPreviewError(`Failed to preview ${themeName}: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  private async installThemeFromRegistry(themeName: string): Promise<void> {
    try {
      ...
      this.showPreviewError(`Failed to install ${themeName}: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
}